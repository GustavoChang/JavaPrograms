...
public class PvInspecaoForm extends Activity implements AdapterView.OnItemSelectedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            getElementosForm();
            setListenerSpinner();
            vwVarreduraCDP = vwVarreduraCDPDAO.getModeloByPK(vwVarredura.getIdVarreduraCDP());
...
//            });
            ...
            ArrayAdapter<TbVarreduraDiag> dataAdapterDiag = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, tbVarreduraDiagDAO.listarByClas(vwVarreduraCDP.getIdVarreduraClas()));
            dataAdapterDiag.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            spdiag.setAdapter(dataAdapterDiag);

            ArrayAdapter<TbVarreduraProg> dataAdapterProg = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, tbVarreduraProgDAO.listarByDiag(vwVarreduraCDP.getIdVarreduraDiag()));
            dataAdapterProg.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            spprog.setAdapter(dataAdapterProg);
                ...
                public void onClick(View arg0) {
                    ...
                    try {
                        if(validaFormulario()){
                            ...
                            Intent i = new Intent(PvInspecaoForm.this, MapaActivity.class);
...
//                        finish();
    ...
    public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
        if(adapterView.getId() == R.id.spretorno){
            ...
            System.out.println("DENTRO DO LISTEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEERRRRRRRRRRRRNE");
...
//                            ".jpg";
    ...
    public void exibirMensagem(String titulo, String texto) {
        ...
        AlertDialog.Builder dialogo = new AlertDialog.Builder(PvInspecaoForm.this);
    ...
    public void montarListCroqui() {
        try{
            ...
            CustomAdapter mCustomAdapter = new CustomAdapter(PvInspecaoForm.this, spinnerTitles, spinnerImages, spinnerPopulation);
    ...
    public void criaSelectedSpinner(){
        ...
        if (vwVarredura.getIdVarreduraRet()!= 0){

            for(int i=0; i<spclas.getCount(); i++){
                retTbVarreduraClas = (TbVarreduraClas) spclas.getItemAtPosition(i);
                if(Long.toString(vwVarreduraCDP.getIdVarreduraClas()).equals(Long.toString(retTbVarreduraClas.getIdVarreduraClas()))){
                    System.out.println(i);
                    spclas.setSelection(i);
                }
            }

            //vwVarreduraCDP = vwVarreduraCDPDAO.getModeloByPK(vwVarredura.getIdVarreduraCDP());
            for(int i=0; i<spdiag.getCount(); i++){
                retTbVarreduraDiag = (TbVarreduraDiag) spdiag.getItemAtPosition(i);
                if(Long.toString(vwVarreduraCDP.getIdVarreduraDiag()).equals(Long.toString(retTbVarreduraDiag.getIdVarreduraDiag()))){
                    System.out.println(i);
                    spdiag.setSelection(i);
                }
            }
            for(int i=0; i<spprog.getCount(); i++){
                retTbVarreduraProg = (TbVarreduraProg) spprog.getItemAtPosition(i);
                if(Long.toString(vwVarreduraCDP.getIdVarreduraProg()).equals(Long.toString(retTbVarreduraProg.getIdVarreduraProg()))){
                    System.out.println(i);
                    spprog.setSelection(i);
                }
            }

            ///
            for(int i=0; i<spcroq.getCount(); i++){
                System.out.println("?????????????????????????????????????Entrou");
                System.out.println(spcroq.getCount());
                retTbVarreduraCroq = (TbVarreduraCroq) spcroq.getItemAtPosition(i);
                System.out.println(i);
                System.out.println(vwVarredura.getIdVarreduraCroq());
                System.out.println(retTbVarreduraCroq.getIdVarreduraCroq());
                if(Long.toString(vwVarredura.getIdVarreduraCroq()).equals(Long.toString(retTbVarreduraCroq.getIdVarreduraCroq()))){
                    System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAENNNNNNNNNNNNTROOOOOOOOOOOUUUUUUUUUUU");
                    System.out.println(i);
                    spcroq.setSelection(i);
                }
            }
            try {
                List<TbVarreduraCroq> list = tbVarreduraCroqDAO.listar();
...
//
//
//        if (vwVarreduraCDP.getIdVarreduraClas()!= 0){
//            vwVarreduraCDP = vwVarreduraCDPDAO.getModeloByPK(vwVarredura.getIdVarreduraCDP());
//            for(int i=0; i<spclas.getCount(); i++){
//                retTbVarreduraClas = (TbVarreduraClas) spdiag.getItemAtPosition(i);
//                if(Long.toString(vwVarreduraCDP.getIdVarreduraClas()).equals(Long.toString(retTbVarreduraClas.getIdVarreduraClas()))){
//                    System.out.println(i);
//                    spclas.setSelection(i);
//                }
